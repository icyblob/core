...
static volatile char icyLock = 0;
static CHAR16 messageDebug[512];
static int debugRepeat = 0;
        ...
        else
        {
            ...
            else
            {
                ...
                case RequestContractFunction::type:
                {
                    ...
                    ACQUIRE(icyLock);
                    debugRepeat = 3;
                    setText(messageDebug, L"AAAAAAAAAAAMessage CONTRACT REQUEST Received!AAAAAAAAAA");
                    RELEASE(icyLock);
                }
                break;
...
static bool processTickTransactionContractProcedure(const Transaction* transaction, const int spectrumIndex, const unsigned int contractIndex)
{
    ...
    ACQUIRE(icyLock);
    debugRepeat = 3;
    appendText(messageDebug, L"\nNext tick data epoch: ");
    appendNumber(messageDebug, nextTickData.epoch, TRUE); 
    appendText(messageDebug, L"\nSystem epoch: ");
    appendNumber(messageDebug, system.epoch, TRUE); 
    appendText(messageDebug, L"\nTransaction not null: ");
    appendNumber(messageDebug, transaction != nullptr, TRUE); 
    appendText(messageDebug, L"\nTransaction validity: ");
    appendNumber(messageDebug, transaction->checkValidity(), TRUE);
    appendText(messageDebug, L"\nTransaction tick: ");
    appendNumber(messageDebug, transaction->tick, TRUE);
    appendText(messageDebug, L"\nsystem tick: ");
    appendNumber(messageDebug, system.tick, TRUE);
    appendText(messageDebug, L"\nSpectrum index: ");
    appendNumber(messageDebug, spectrumIndex, TRUE);
    appendText(messageDebug, L"\nContract index: ");
    appendNumber(messageDebug, contractIndex, TRUE);
    appendText(messageDebug, L"\nContract count: ");
    appendNumber(messageDebug, contractCount, TRUE);
    appendText(messageDebug, L"\nConstruction epoch: ");
    appendNumber(messageDebug, contractDescriptions[contractIndex].constructionEpoch, TRUE);
    appendText(messageDebug, L"\nDestruction epoch: ");
    appendNumber(messageDebug, contractDescriptions[contractIndex].destructionEpoch, TRUE);
    RELEASE(icyLock);
...
static void logInfo()
{
    ...
    if (debugRepeat > 0)
    {
        logToConsole(messageDebug);
        debugRepeat--;
    }
    else
    {
        debugRepeat = 0; 
        setText(messageDebug, L"");
    }
    RELEASE(icyLock);
